                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="<?=BASE_URL('')?>">Danh sách tài khoản ACB</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <div class="d-flex justify-content-end">
                                    <a href="<?=BASE_URL('client/add-account-acb')?>" class="btn btn-success btn-sm">Thêm ACB</a>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-body table-border-style">
                        <div class="table-responsive">
                            <table class="table w-100" id="datatable">
                                ...
                                <tbody>
                                    <?php foreach ($NNL->get_list("SELECT * FROM `account_acb` WHERE `user_id`='" . $getUser['id'] . "'") as $row) { ?>
                                        <tr>
                                            <td><?= $row['phone'] ?></td>
                                            <td><?= $row['stk'] ?></td>
                                            <td><?= $row['name'] ?></td>
                                           <td class="money-acb" data-token="<?= $row['token'] ?>">Đang tải...</td>
                                            ...
                                            <td class="table-action">
                                                ...
                                                <a href="<?= BASE_URL('client/viewhisacb/'), $row['stk'] ?>"><button class="btn btn-success btn-xs" type="button"><i class="fa fa-list"></i> Lịch sử giao dịch</button></a>
...
$('a').on('click', function(e) {
	...
	function updateBalances() {
		$('.money-acb').each(function() {
			var element = $(this);
			var token = element.data('token');
			var apiUrl = "<?= BASE_URL('api/historyacbbalance/') ?>" + token;
    ...
    function Delete(id) {
        cuteAlert({
            ...
            if (e) {
                $.ajax({
                    type: "post",
                    url: "<?= BASE_URL("ajaxs/client/acb.php"); ?>",
                    dataType: "json",
                    data: {
                        action: "REMOVE",
                        id: id,
	...
	function format_cash(value) {
    return value.toLocaleString('en-US'); // Sử dụng định dạng tiếng Anh (Hoa Kỳ) cho dấu phẩy hàng nghìn
}
