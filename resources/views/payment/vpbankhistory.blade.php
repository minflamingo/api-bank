...
@section('title', 'Lịch sử giao dịch VPBank')

@section('content')
@php
  $valueOf = function ($item, array $keys, $default = '') {
      foreach ($keys as $key) {
...
<div data-bank-history-page>
  <div data-bank-history-content>
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
      <div>
        <div class="text-muted small mb-1">VPBank</div>
          ...
          <tbody>
            @forelse($transactions as $item)
              @php
                ...
                $reference = $item['_reference'] ?? $valueOf($item, ['Reference', 'ReferenceNumber', 'TransactionId', 'Id', 'TransactionNumber'], '');
                $description = $item['_description'] ?? $valueOf($item, ['Description', 'TransactionDescription', 'Narrative', 'Remark', 'Content'], '');
              @endphp
              <tr>
                <td>{{ $dateText ?: '-' }}</td>
                <td>
                ...
                <td class="{{ $isCredit ? 'text-success' : 'text-danger' }}">{{ number_format($amount) }}đ</td>
