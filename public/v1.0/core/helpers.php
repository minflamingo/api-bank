...
function takeimageqr($sotaikhoan,$chutaikhoan,$chinhanh,$magd,$template)
		...
		$headers = array(
			...
			'x-client-id: 3c909ee0-dea9-41a4-81e7-8b5b2f548e24', // Thay thế YOUR_CLIENT_ID_HERE bằng Client Id thực
			'x-api-key: 1b113c4d-ed39-4ff2-a56d-86f4f47ff713'      // Thay thế YOUR_API_KEY_HERE bằng API Key thực
		);

		// Khai báo dữ liệu cần gửi trong request
		$data = array(
		...
		// Xử lý kết quả $response tại đây
		//echo $response;

		// Giải mã kết quả trả về từ JSON
		$response_data = json_decode($response, true);

		// Lấy giá trị qrDataURL từ kết quả
		$qrDataURL = $response_data['data']['qrDataURL'] ?? '';

		// Hiển thị hình ảnh QR nếu có
		//if (!empty($qrDataURL)) {
		//	echo '<img src="' . $qrDataURL . '" alt="QR Code" width=300/>';
		//} else {
		//	echo 'Không thể lấy dữ liệu QR Code!';
		//}
		return $qrDataURL;
...
function yugetpost($paramName) {
    if (isset($_GET[$paramName])) {
        return $_GET[$paramName];
    } elseif (isset($_POST[$paramName])) {
        return $_POST[$paramName];
    }
    return null; // hoặc có thể trả về một giá trị mặc định nếu thích
}
...
function odertele($text) 
{
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.telegram.org/bot" . $NNL->site('token_tele') . "/sendMessage?chat_id=" . $NNL->site('id_tele') . "&text=" . urlencode($text),
...
function getMoney_vcb($token)
{
    ...
    $result = curl_get(BASE_URL('') . "api/historyvietcombankbalance/" . $token);
    $result = json_decode($result, true);
...
{
    $result = curl_get(BASE_URL('') . "api/historymbbankbalance/" . $token);
    $result = json_decode($result, true);
...
function getMoney_acb($token)
{

    $result = curl_get(BASE_URL('') . "api/historyacbbalance/" . $token);
    $result = json_decode($result, true);
