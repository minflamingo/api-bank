<?php
define("IN_SITE", true);
require_once "../../core/DB.php";
require_once "../../core/helpers.php";
require_once '../../core/class/class.smtp.php';
require_once '../../core/class/PHPMailerAutoload.php';
require_once '../../core/class/class.phpmailer.php';
require_once "../../core/class/mbbank.php";
$mbbank = new MBBANK;
if ($_SERVER['REQUEST_METHOD'] == 'GET' || $_SERVER['REQUEST_METHOD'] == 'POST') {
    if (yugetpost('action') == 'LOGIN') {
        if (empty(yugetpost('token'))) {
            die(json_encode(['status' => '1', 'msg' => 'Vui lòng đăng nhập']));
        }
        if (!$getUser = $NNL->get_row("SELECT * FROM `users` WHERE `token` = '" . xss(yugetpost('token')) . "' AND `banned` = '0' ")) {
            die(json_encode(['status' => '1', 'msg' => 'Vui lòng đăng nhập']));
        }
        $account = xss(yugetpost('account'));
        $password = xss(yugetpost('password'));
        $stk = xss($_POST['stk']);
        if (empty($account)) {
            die(json_encode([
                'status' => '1',
                'msg' => 'Vui lòng nhập tài khoản mbbank',
            ]));
        }
        if (empty($password)) {
            die(json_encode([
                'status' => '1',
                'msg' => 'Vui lòng nhập mật khẩu mbbank',
            ]));
        }
        if (empty($stk)) {
            die(json_encode([
                'status' => '1',
                'msg' => 'Vui lòng nhập số tài khoản mbbank',
            ]));
        }
        if ($getUser['time_momo'] < time()) {
            die(json_encode([
                'status' => '1',
                'msg' => 'Gói API của bạn đã hết hạn sử dụng, vui lòng nâng cấp để tiếp tục sử dụng',
            ]));
        }
        $token = md5(random('QWERTYUIOPASDGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm0123456789', 6) . time());
        $mbbank->user = $account; 
        $mbbank->pass = $password; 
        $text_captcha = $mbbank->bypass_captcha_stc($NNL->site('key_captcha'));
        $login = json_decode($mbbank->login($text_captcha), true); //responseCode 283 lỗi captcha, GW21 thông tin sai
        $mbbank->deviceIdCommon_goc = $mbbank->generateImei();
        if ($login['result']['message'] == "Capcha code is invalid") {
            exit(json_encode(array('status' => '1', 'msg' => 'Captcha không chính xác')));
        } else if ($login['result']['message'] == 'OK') {
			 $create = $NNL->insert("account_mbbank", [
                'user_id' => $getUser['id'],
                'phone' => $account,
                'stk' => $stk,
                'name' => $login['cust']['nm'],
                'password' => $password,
                'sessionId' => $login['sessionId'],
                'deviceId' => $mbbank->deviceIdCommon_goc,
                'token' => $token,
                'time' => time(),
            ]);
            exit(json_encode(array('status' => '2', 'msg' => 'Thêm tài khoản thành công')));
            
        } else {
            exit(json_encode(array('status' => '1', 'msg' => 'Thông tin không chính xác')));

        }
    }

    if (yugetpost('action') == 'SENDTOKEN') {
        if (empty(yugetpost('token'))) {
            die(json_encode(['status' => '1', 'msg' => 'Vui lòng đăng nhập']));
        }
        if (!$getUser = $NNL->get_row("SELECT * FROM `users` WHERE `token` = '" . xss(yugetpost('token')) . "' AND `banned` = '0' ")) {
            die(json_encode(['status' => '1', 'msg' => 'Vui lòng đăng nhập']));
        }
        $id = xss($_POST['id']);
        if (empty($id)) {
            exit(json_encode(array('status' => '1', 'msg' => 'Vui lòng điền thông tin!')));
        }
        $row = $NNL->get_row(" SELECT * FROM `account_mbbank` WHERE `id` = '$id' AND `user_id`='" . $getUser['id'] . "' ");
        if (!$row) {
            exit(json_encode(array('status' => '1', 'msg' => 'Tài khoản mbbank không tồn tại!')));
        }
        $user = $NNL->get_row(" SELECT * FROM `users` WHERE `id` = '" . $row['user_id'] . "' ");
        if (!$user) {
            exit(json_encode(array('status' => '1', 'msg' => 'Người dùng không tồn tại!')));
        }

        $guitoi = $user['email'];
        $subject = 'TOKEN MBBANK';
        $bcc = "API SYSTEM";
        $hoten = 'Client';
        $token = $row['token'];
        $noi_dung = '<h3>Có ai đó vừa yêu cầu gửi token Mbbank bằng Email này, nếu là bạn thì token bên dưới dùng để chạy api</h3>
						<table>
						<tbody>
						<tr>
							<td style="font-size:20px;">TOKEN:</td>
							<td><b style="color:blue;font-size:30px;">' . $token . '</b></td>
						</tr>
						<tr><td colspan="2"><br></td></tr>
						<tr>
							<td style="font-size:20px;">LINK API GIAO DỊCH:</td>
							<td><b style="color:blue;font-size:30px;">' . BASE_URL('') . "api/historymbbank/" . $token . '</b></td>
						</tr>
						<tr><td colspan="2"><br></td></tr>
						<tr>
							<td style="font-size:20px;">LINK API SỐ DƯ:</td>
							<td><b style="color:blue;font-size:30px;">' . BASE_URL('') . "api/historymbbankbalance/" . $token . '</b></td>
						</tr>
						</tbody>
						</table>';
        Locdz_Email($guitoi, $hoten, $subject, $noi_dung, $bcc);
        exit(json_encode(array('status' => '2', 'msg' => 'Đã gửi token đến mail của bạn!')));
    }
    //get name
    if (yugetpost('action') == 'REMOVE') {
        if (empty(yugetpost('token'))) {
            die(json_encode(['status' => '1', 'msg' => 'Vui lòng đăng nhập']));
        }
        if (!$getUser = $NNL->get_row("SELECT * FROM `users` WHERE `token` = '" . xss(yugetpost('token')) . "' AND `banned` = '0' ")) {
            die(json_encode(['status' => '1', 'msg' => 'Vui lòng đăng nhập']));
        }
        $id = xss($_POST['id']);
        if (empty($id)) {
            exit(json_encode(array('status' => '1', 'msg' => 'Không được!')));
        }
        $tool = $NNL->get_row(" SELECT * FROM `account_mbbank` WHERE `id` = '$id' AND `user_id`='" . $getUser['id'] . "' ");
        if (!$tool) {
            exit(json_encode(array('status' => '1', 'msg' => 'Định hack à không dễ vậy đâu!')));
        }
        $NNL->remove("account_mbbank", "`id`='" . $id . "'");
        exit(json_encode(array('status' => '2', 'msg' => 'Đã xóa tài khoản thành công!')));
    }
}
