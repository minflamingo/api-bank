                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="<?=BASE_URL('')?>">Danh sách tài khoản Vietcombank</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <div class="d-flex justify-content-end">
                                    <a href="<?=BASE_URL('client/add-account-vcb')?>" class="btn btn-success btn-sm">Thêm Vietcombank</a>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-body table-border-style">
                        <div class="table-responsive">
                            <table class="table w-100" id="datatable">
                                ...
                                <tbody>
                                    <?php foreach ($NNL->get_list("SELECT * FROM `account_vietcombank` WHERE `user_id`='" . $getUser['id'] . "'") as $row) { ?>
                                        <tr>
											<td><?= $row['username'] ?></td>
											<td><?= $row['name'] ?></td>
                                            <td><?= $row['account'] ?></td>
                                            ...
                                            <td class="table-action">
                                                ...
                                                <a href="<?= BASE_URL('client/viewhisvcb/'), $row['account'] ?>"><button class="btn btn-success btn-xs" type="button"><i class="fa fa-list"></i> Lịch sử giao dịch</button></a>
...
<script type="text/javascript">
	...
	function abortAllAjaxRequests() {
		...
		ajaxRequests = []; // Xóa mảng sau khi hủy bỏ các yêu cầu
	}
...
$('a').on('click', function(e) {
	...
	function updateBalances() {
		$('.money-vietcombank').each(function() {
			var element = $(this);
			var token = element.data('token');
			var apiUrl = "<?= BASE_URL('api/historyvietcombankbalance/') ?>" + token;
    ...
    function Delete(id) {
        cuteAlert({
            ...
            if (e) {
                $.ajax({
                    type: "post",
                    url: "<?= BASE_URL("ajaxs/client/vcb.php"); ?>",
                    dataType: "json",
                    data: {
                        action: "REMOVE",
                        id: id,
